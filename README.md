...
instead of ~2 MB). Verse numbers an edition merges away still resolve through the
passages endpoint, so offering 1…highest is safe. CUV is served entirely from
...
it cannot. A range spans at most **8** verses.
...
licensed edition fetches one extra well-known passage from a different book.
...
Hard; the catalogue never offers a verse an edition merged away;
